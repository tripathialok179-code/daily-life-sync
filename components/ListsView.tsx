...
import { Plus, MoreVertical, Trash, GripVertical, X, Check, Wand2 } from 'lucide-react';
...
const ListsView: React.FC<ListsViewProps> = ({ lists, setLists }) => {
  ...
  const handleGenerateItems = async (list: CustomList) => {
    setIsGenerating(true);
    const items = await generateSubtasks(list.title);
    if (items.length > 0) {
       setLists(prev => prev.map(l => {
         if (l.id === list.id) {
           const newItems = items.map(text => ({ id: crypto.randomUUID(), text, checked: false }));
          ...
          >
            <div className="p-6 border-b border-gray-100/50 dark:border-gray-700/50 flex justify-between items-center bg-white/40 dark:bg-gray-800/40 backdrop-blur-md">
              ...
              <div className="flex items-center gap-1">
                 <button 
                   onClick={() => handleGenerateItems(list)}
                   disabled={isGenerating}
                   title="Auto-generate items"
                   className="relative overflow-hidden p-2 text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-900/20 rounded-full transition-colors"
                 >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3 no-scrollbar">
              {list.items.length === 0 ? (
                 <div className="h-full flex flex-col items-center justify-center text-gray-400 text-sm text-center p-4 opacity-60">
                   <p>Empty list.</p>
                   <p className="text-xs mt-1">Add items or use the Wand.</p>
                 </div>
              ) : (
                 list.items.map(item => (
                  <div key={item.id} className="group flex items-center gap-3 p-2 pr-4 rounded-full bg-white/30 dark:bg-gray-800/30 hover:bg-white/50 dark:hover:bg-gray-800/50 transition-colors relative overflow-hidden">
                    <button
                      ...
                      className={`relative flex-shrink-0 w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all duration-200 overflow-hidden ${
                        item.checked 
